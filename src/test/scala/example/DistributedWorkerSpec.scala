...
object DistributedWorkerSpec {
  class FlakyWorkExecutor extends Actor {
    ...
    def receive = {
      case n: Int =>
        i += 1
        if (i == 3) throw new RuntimeException("Flaky example")
        if (i == 5) context.stop(self)

        val n2 = n * n
        val result = s"$n * $n = $n2"
        println(s"Flaky Handling ${n}")
...
class DistributedWorkerSpec extends Specification with NoTimeConversions {
  ...
  "Distributed workers" should {
    "perform work and publish results via frontend" in new test.AkkaTestkitSpecs2Support {
      val clusterAddress = Cluster(system).selfAddress
      Cluster(system).join(clusterAddress)
      system.actorOf(ClusterSingletonManager.props(_ => Broker.props(BrokerConfig.default), "active",
      ...
      for (n <- 1 to 3)
        system.actorOf(Worker.props(clusterClient, Props[DummyWorkExecutor], 1.second), "example-" + n)
      val flakyWorker = system.actorOf(Worker.props(clusterClient, Props[FlakyWorkExecutor], 1.second), "flaky-example")

      val frontend = system.actorOf(Props[Frontend], "frontend")

      val results = TestProbe()
      DistributedPubSubExtension(system).mediator ! Subscribe(BrokerConfig.default.resultTopic, results.ref)
